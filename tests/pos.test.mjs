...
test('01 Â· cenÃ¡rio R$100 / R$50 / R$5 / R$45 / troco R$5 / saldo R$145',t=>{
...
test('02 Â· repetir a venda devolve o mesmo resultado sem novos efeitos',t=>{
...
test('03 Â· mesma chave com valores diferentes Ã© recusada',t=>{
...
test('04 Â· resposta perdida pode ser recuperada por chave',t=>{
...
test('05 Â· falha antes de COMMIT desfaz venda, pagamento, estoque, caixa, auditoria e chave',t=>{
...
test('06 Â· estoque insuficiente nÃ£o gera registros parciais',t=>{
...
test('07 Â· venda em caixa fechado Ã© recusada',t=>{
...
test('08 Â· operador sem permissÃ£o nÃ£o concede desconto',t=>{
  const {pos}=fixture(t);const cash=open(pos,DEMO.cashier);
  fails(()=>pos.sell(DEMO.cashier,key(),saleInput(cash)),'DISCOUNT_FORBIDDEN');
});
test('09 Â· operador vende sem desconto em seu prÃ³prio caixa',t=>{
  const {pos}=fixture(t);const cash=open(pos,DEMO.cashier);
...
test('10 Â· gerente precisa justificar desconto e respeitar limite',t=>{
...
test('15 Â· preÃ§o, total e tenant enviados pelo navegador sÃ£o rejeitados',t=>{
  const {pos}=fixture(t);const cash=open(pos);
...
test('16 Â· valores fracionÃ¡rios, negativos, NaN e quantidades repetidas sÃ£o recusados',t=>{
  const {pos}=fixture(t);const cash=open(pos);
...
test('17 Â· dinheiro insuficiente Ã© recusado sem baixa',t=>{
  const {pos}=fixture(t);const cash=open(pos);
...
test('17b - PIX e cartao registram venda sem alterar dinheiro esperado do caixa',t=>{
  const {pos,db}=fixture(t);const cash=open(pos);
  const pix=pos.sell(DEMO.manager,key(),saleInput(cash,{paymentMethod:'PIX',discountCents:0,discountReason:null,tenderedCents:0})).data;
  assert.equal(pix.payment.method,'PIX');assert.equal(pix.payment.tendered_cents,5000);assert.equal(pix.payment.change_cents,0);
  assert.equal(pos.cash(DEMO.manager,cash.id).expected_cents,10000);
  const card=pos.sell(DEMO.manager,key(),saleInput(cash,{paymentMethod:'CARD',items:[{productId:DEMO.product,quantity:1}],discountCents:0,discountReason:null,tenderedCents:0})).data;
  assert.equal(card.payment.method,'CARD');assert.equal(card.payment.amount_cents,2500);
  assert.equal(pos.cash(DEMO.manager,cash.id).expected_cents,10000);
...
test('18 Â· reabertura simultÃ¢nea do mesmo terminal nÃ£o cria dois caixas',t=>{
  const {pos}=fixture(t);open(pos);fails(()=>open(pos),'CASH_ALREADY_OPEN');
});
test('19 Â· abertura e fechamento tambÃ©m sÃ£o idempotentes',t=>{
...
test('20 Â· outro operador nÃ£o vende nem fecha o caixa do gerente',t=>{
  const {pos}=fixture(t);const cash=open(pos);
...
test('21 Â· fechamento com diferenÃ§a exige motivo e registra divergÃªncia',t=>{
  ...
  assert.equal(pos.closeCash(DEMO.manager,key(),{...body,reason:'DiferenÃ§a fictÃ­cia para teste'}).data.difference_cents,-500);
});
test('22 Â· cadastro mantÃ©m movimento inicial e impede cÃ³digo duplicado',t=>{
...
test('23 Â· alteraÃ§Ã£o posterior de cadastro nÃ£o muda snapshot de venda',t=>{
...
test('24 Â· registros confirmados nÃ£o podem ser apagados ou alterados',t=>{
...
test('25 Â· chaves estrangeiras compostas bloqueiam mistura de empresas',t=>{
  const {db}=fixture(t);
  assert.throws(()=>db.prepare('INSERT INTO stock VALUES(?,?,?,?)').run('tenant-demo',DEMO.storeA,'product-other',3),/FOREIGN KEY/);
  assert.equal(db.prepare('PRAGMA foreign_key_check').all().length,0);
});
test('26 Â· reinÃ­cio preserva venda e seed nÃ£o repÃµe saldo',t=>{
...
test('27 Â· conversÃ£o monetÃ¡ria por centavos digitados, decimal explÃ­cito e limites',()=>{
...
test('28 Â· mesma chave nÃ£o expÃµe resposta para outro operador',t=>{
...
test('29 Â· desativar usuÃ¡rio bloqueia acesso mesmo com contexto previamente obtido',t=>{
...
test('30 Â· duas conexÃµes reais disputam a Ãºltima unidade: apenas uma confirma',async t=>{
  const {pos,db,filename}=fixture(t,{file:true});const cash1=open(pos),cash2=open(pos,DEMO.manager,DEMO.terminalA2);
  const product=pos.createProduct(DEMO.manager,key(),{storeId:DEMO.storeA,sku:'LAST',name:'Ãšltima unidade',priceCents:100,initialQuantity:1}).data;
...
test('31 Â· transaÃ§Ã£o nÃ£o aceita funÃ§Ã£o assÃ­ncrona',t=>{
  const {db}=fixture(t);assert.throws(()=>transaction(db,()=>Promise.resolve()),/assincrona/);
