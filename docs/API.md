...
Subtotal, preço e total são calculados no servidor. `priceCents`, `totalCents`, `tenantId` e outros campos desconhecidos são recusados. O gerente pode conceder desconto até 20%; operador não pode. Quantidades repetidas do mesmo produto devem estar agrupadas em uma linha.
...
A consulta de operação não expõe a resposta de outro operador. A visualização de venda depende de autorização para sua loja. Não há rotas de cancelamento, devolução, PIX, cartão ou emissão fiscal.
