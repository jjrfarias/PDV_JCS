...
if(major<22||(major===22&&minor<16)) throw new Error('Use Node.js 22.16 ou superior. Para instalaÃ§Ã£o nova, use a linha LTS 24.');
...
if(production && process.env.DATABASE_ENGINE!=='postgres') throw new Error('ProduÃ§Ã£o exige DATABASE_ENGINE=postgres.');
let db;
let closeDb;
let accessFile;
if(usePostgres) {
  ...
  if(credentials) {
    const content='ACESSOS LOCAIS DE TESTE. NÃƒO UTILIZAR EM PRODUÃ‡ÃƒO.\n\n'+credentials.map(u=>`Empresa: ${u.tenant}\nE-mail: ${u.email}\nSenha: ${u.password}\n`).join('\n');
    // Arquivo nÃ£o servido por HTTP; permissÃµes POSIX restritas, nÃ£o equivalentes a ACL do Windows.
    writeFileSync(accessFile,content,{mode:0o600,flag:'wx'});
    console.log(content);
  }
}
const server=createApp(db);
server.on('error',error=>{ console.error('NÃ£o foi possÃ­vel iniciar:',error.code??error.message); closeDb(); process.exitCode=1; });
server.listen(port,production?'0.0.0.0':'127.0.0.1',()=>{
  ...
  console.log(usePostgres?'PostgreSQL ativo.':'Laboratorio local SQLite. Sem PIX automatico, cartao integrado ou emissao fiscal.');
});
let stopping=false;
